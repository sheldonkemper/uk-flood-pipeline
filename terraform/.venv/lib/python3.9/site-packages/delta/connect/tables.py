...
class DeltaTable(object):
    ...
    def delete(self, condition: OptionalExpressionOrColumn = None) -> DataFrame:
        ...
        df = DataFrame.withPlan(plan, session=self._spark)
    ...
    def history(self, limit: Optional[int] = None) -> DataFrame:
        df = DataFrame.withPlan(DescribeHistory(self._to_proto()), session=self._spark)
    ...
    def detail(self) -> DataFrame:
        return DataFrame.withPlan(DescribeDetail(self._to_proto()), session=self._spark)
...
class DeltaTableBuilder(object):
    ...
    def addColumn(
        ...
        if generatedAlwaysAs is not None and type(generatedAlwaysAs) is not str:
            self._raise_type_error(
                "Generated always as expression must be str.", [generatedAlwaysAs])
